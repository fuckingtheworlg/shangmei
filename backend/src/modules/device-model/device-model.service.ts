...
export class DeviceModelService {
  ...
  create(data: { name: string; spec?: string; unit?: string; categoryId?: number; remark?: string }) {
    return this.prisma.deviceModel.create({ data });
  }

  async update(
    id: number,
    data: { name?: string; spec?: string; unit?: string; categoryId?: number | null; remark?: string },
  ) {
